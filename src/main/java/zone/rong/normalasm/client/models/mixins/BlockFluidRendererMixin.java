package zone.rong.normalasm.client.models.mixins;
...
public class BlockFluidRendererMixin {
    ...
    /**
     * @author Rongmario
     * @reason Decrease amount of checks done when rendering fluids
     */
    /*
    @Overwrite
    public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPos blockPosIn, BufferBuilder bufferBuilderIn) {
        if (blockStateIn.shouldSideBeRendered(blockAccess, blockPosIn, EnumFacing.UP)) { // flag1 - UP
            float slopeAngle = BlockLiquid.getSlopeAngle(blockAccess, blockPosIn, blockStateIn.getMaterial(), blockStateIn);
            TextureAtlasSprite sprite = slopeAngle > -999.0F ? this.atlasSpritesLava[1] : this.atlasSpritesWater[0];
        }
