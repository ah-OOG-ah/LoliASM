package zone.rong.normalasm.common.capability.astralsorcery.mixins;
...
import zone.rong.normalasm.api.IItemStackCapabilityInjector;
import zone.rong.normalasm.api.ICapabilityDispatcherManipulator;
...
public class EnchantmentUpgradeHelperMixin {
    ...
    public static void applyAmuletOwner(ItemStack tool, EntityPlayer wearer) {
        ...
        if (cap == null) {
            ...
            ((AmuletHolderCapabilityProviderAccessor) provider).normalasm$defaultImplFastAccess().setHolderUUID(wearer.getUniqueID());
        } else {
