...
public final class CrashUtils {
    ...
    public static void warn(CrashReport report) {
        if (FMLLaunchHandler.side().isClient()) {
            ...
            NormalLogger.instance.fatal(report.getDescription(), report.getCrashCause());
        }
    }
    ...
    public static void outputReport(CrashReport report) {
        ...
        NormalLogger.instance.fatal("Minecraft ran into a problem! " + (report.getFile() != null ? "Report saved to: " + report.getFile() : "Crash report could not be saved.") + "\n" + report.getCompleteReport());
    }
