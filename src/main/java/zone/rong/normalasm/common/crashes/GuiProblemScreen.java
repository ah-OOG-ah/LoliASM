...
import zone.rong.normalasm.api.HasteUpload;
...
public abstract class GuiProblemScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.clear();
        buttonList.add(new GuiButton(1, width / 2 - 155 + 160, height / 4 + 120 + 12, 150, 20, I18n.format("normalasm.gui.getLink")));
    }
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 1) {
            try {
                ...
                NormalLogger.instance.error("Exception when crash menu button clicked:", e);
                button.displayString = I18n.format("normalasm.gui.failed");
    ...
    protected String getModListString() {
        if (modListString == null) {
            ...
            if (suspectedMods == null) {
                return modListString = I18n.format("normalasm.crashscreen.identificationErrored");
            }
            ...
            if (modNames.isEmpty()) {
                modListString = I18n.format("normalasm.crashscreen.unknownCause");
            } else {
