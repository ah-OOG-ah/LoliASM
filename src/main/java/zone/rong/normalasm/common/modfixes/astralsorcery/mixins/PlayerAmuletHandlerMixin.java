package zone.rong.normalasm.common.modfixes.astralsorcery.mixins;
...
public abstract class PlayerAmuletHandlerMixin implements ITickHandler {
    ...
    public void onPlayerDeath(PlayerDropsEvent event) {
        ...
        for (ItemStack armorStack : player.getArmorInventoryList()) {
            EnchantmentUpgradeHelperInvoker.normalasm$removeAmuletOwner(armorStack);
        }
        EnchantmentUpgradeHelperInvoker.normalasm$removeAmuletOwner(player.getHeldItemMainhand());
        EnchantmentUpgradeHelperInvoker.normalasm$removeAmuletOwner(player.getHeldItemOffhand());
    }
