...
import zone.rong.normalasm.UnsafeNormals;
import zone.rong.normalasm.config.NormalConfig;
import zone.rong.normalasm.NormalLogger;
import zone.rong.normalasm.spark.NormalSparker;
import zone.rong.normalasm.api.DeobfuscatingRewritePolicy;
import zone.rong.normalasm.api.StacktraceDeobfuscator;
...
public class NormalLoadingPlugin implements IFMLLoadingPlugin, IEarlyMixinLoader {
    ...
    public NormalLoadingPlugin() {
        NormalLogger.instance.info("Normal people are on the {}-side.", isClient ? "client" : "server");
        NormalLogger.instance.info("Normal people are preparing and loading in mixins since Rongmario's too lazy to write pure ASM at times despite the mod being called 'NormalASM'");
        ...
        if (NormalConfig.instance.removeForgeSecurityManager) {
            UnsafeNormals.removeFMLSecurityManager();
        }
    ...
    public String getSetupClass() {
        return "zone.rong.normalasm.core.NormalFMLCallHook";
    }
    ...
    public String getAccessTransformerClass() {
        return "zone.rong.normalasm.core.NormalTransformer";
    }
